...
//        constraints.gridx++;
    ...
    private void DrawFigures(GridBagConstraints constraints) {
        for (int i = 0; i < figures.length; i++) {
            constraints.anchor = GridBagConstraints.SOUTH;
            constraints.fill = GridBagConstraints.BOTH;
            constraints.gridx = 21 - figures.length + i;
            constraints.gridwidth = 1;
            constraints.gridy = 0;
    ...
    public void handle(Event e) {
        ...
        if (e instanceof TeamStatsMessage) {
            changedstats = ((TeamStatsMessage) e).getStats();
            RefreshPanel();
        }
    }
    ...
    private void RefreshPanel() {
        ...
        CreateStats();
        WriteStats(new GridBagConstraints());
        this.repaint();
    }
    ...
    private void CreateStats() {
        ...
        constraints.gridx = 21 - figures.length;
        constraints.gridwidth = 1;
        constraints.gridheight = 1;
        for (int i = 0; i < changedstats.size(); i++) {
            ...
            for (int j = 1; j < changedstats.get(i).length; j++) {
                ...
                figure_image.setFont(new Font("Comic Sans", BOLD, 14));
                ...
                figure_image.addMouseListener(new MouseAdapter() {
                    ...
                    public void mouseClicked(MouseEvent e) {
                        ...
                        RefreshPanel();
                        TeamPanel.this.notify(new ChangeStatsRequest(changedstats));
                    }
                });
                constraints.gridy +=1;
            }
    ...
    private void setNewValue(int i, int j) {
        ...
        if (j == 4){
            char last = changedstats.get(i)[j].charAt(changedstats.get(i)[j].length()-1);
            StringBuilder new_distance = new StringBuilder();
            int new_max_distance = last;
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) {
                    ...
                    delete.addMouseListener(new MouseAdapter() {
                        @Override
                        public void mouseClicked(MouseEvent e) {
                            TeamPanel.this.notify(new DeleteTeamRequest(teams.get(i)));
                            team_dialog.dispose();
                        }
                    });
                    rename.addMouseListener(new MouseAdapter() {
                        ...
                        public void mouseClicked(MouseEvent e) {
                            ...
                            new_name.addActionListener(e1 -> {
                                ...
                                TeamPanel.this.notify(new ReplaceNameRequest(teams.get(i),name_written+".txt"));
    ...
    private void RequestTeamFigures(String text) {
        notify(new TeamStatsRequest(text + ".txt"));
    }
